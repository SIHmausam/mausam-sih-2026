...
class PreferenceService:
    ...
    async def complete_onboarding(
        ...
        preference = existing or UserPreference(
            user_id=user_id
        )
        ...
        preference.temperature_unit = (
            payload.temperature_unit.value
        )
        ...
        preference.rain_alerts_enabled = (
            payload.notifications.rain_alerts
        )

        preference.aqi_alerts_enabled = (
            payload.notifications.aqi_alerts
        )
        ...
        try:
            if existing is None:
                self.repository.add_preference(
                    preference
                )
    ...
    async def get_preferences(
        ...
        preference = (
            await self.repository.get_preference(
                user_id
            )
        )
        ...
        interests = (
            await self.repository.get_interests(
                user_id
            )
        )

        activities = (
            await self.repository.get_activity_preferences(
                user_id
            )
        )
    ...
    async def update_preferences(
        ...
        try:
            ...
            if payload.notifications is not None:
                notification_fields = (
                    payload.notifications.model_fields_set
                )

                if (
                    "official_alerts"
                    in notification_fields
                    and payload.notifications.official_alerts
                    is not None
                ):
                    preference.official_alerts_enabled = (
                        payload.notifications.official_alerts
                ...
                if (
                    "routine_alerts"
                    in notification_fields
                    and payload.notifications.routine_alerts
                    is not None
                ):
                    preference.routine_alerts_enabled = (
                        payload.notifications.routine_alerts
                ...
                if (
                    "rain_alerts"
                    in notification_fields
                    and payload.notifications.rain_alerts
                    is not None
                ):
                    preference.rain_alerts_enabled = (
                        payload.notifications.rain_alerts
                ...
                if (
                    "aqi_alerts"
                    in notification_fields
                    and payload.notifications.aqi_alerts
                    is not None
                ):
                    preference.aqi_alerts_enabled = (
                        payload.notifications.aqi_alerts
                ...
                if (
                    "daily_summary"
                    in notification_fields
                    and payload.notifications.daily_summary
                    is not None
                ):
                    preference.daily_summary_enabled = (
                        payload.notifications.daily_summary
            ...
            if payload.personalization is not None:
                personalization_fields = (
                    payload.personalization.model_fields_set
                )

                if (
                    "personalized_homepage"
                    in personalization_fields
                    and payload.personalization.personalized_homepage
                    is not None
                ):
                    preference.personalized_homepage_enabled = (
                        payload.personalization.personalized_homepage
                ...
                if (
                    "routine_impact"
                    in personalization_fields
                    and payload.personalization.routine_impact
                    is not None
                ):
                    preference.routine_impact_enabled = (
                        payload.personalization.routine_impact
                ...
                if (
                    "learn_from_activity"
                    in personalization_fields
                    and payload.personalization.learn_from_activity
                    is not None
                ):
                    preference.learning_enabled = (
                        payload.personalization.learn_from_activity
            ...
            if (
                "interests" in fields_set
                and payload.interests is not None
            ):
                interests = [
                    UserWeatherInterest(
                        user_id=user_id,
